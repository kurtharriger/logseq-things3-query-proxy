...
(defn things-db [path]
  (let [homedir (j/call os :homedir)
        ...
        exists? #(j/call fs :existsSync path)]
    {:sql-db-exists? exists?
     :sql-db-path path
     :sql-db (delay (if (exists?)
                      (sql/Database. path)
                      (throw (ex-info "database not found" {:path path}))))
     :datascript-conn (create-datascript-conn)
     :listener-chans (atom {})}))

(defn things-db-exists? [things-db] ((:sql-db-exists? things-db (constantly false))))
(defn sql-db [things-db] @(:sql-db things-db))

(defn listen! [things-db key]
